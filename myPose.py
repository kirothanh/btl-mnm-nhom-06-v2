...
class myPose():
    ...
    def checkPose_Clap(self, image, results):
        ...
        if distance < clap_threshold:
            ...
            CLAP = "N"
        adistan = str(distance)
        cv2.putText(image, adistan, (100, 30), cv2.FONT_HERSHEY_PLAIN, 2, (255, 255, 0), 3)
        cv2.putText(image, CLAP, (10, 30), cv2.FONT_HERSHEY_PLAIN, 2, (255, 255, 0), 3)

        return image, CLAP

    def checkPose_HAND(self, image, results):

        image_height, image_width, _ = image.shape

        left_hand = (results.pose_landmarks.landmark[self.mp_pose.PoseLandmark.RIGHT_INDEX].x * image_width,
                     results.pose_landmarks.landmark[self.mp_pose.PoseLandmark.LEFT_WRIST].y * image_height)
